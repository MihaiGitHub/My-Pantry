...
try{
	...
	if(isset($_POST['dateOfVisit']) && isset($_POST['program']) && isset($_POST['volunteer'])){
		...
		$stmt = $objDb->prepare('INSERT INTO visits (`date_of_visit`, `lname`, `fname`, `how_many_in_house`, `phone`, `email`, `program`, `volunteer`, `numBags`, `weight`, `client_id`) 
						VALUES (:dateOfVisit, :lname, :fname, :inHouse, :phone, :email, :program, :volunteer, :numBags, :weight, :id)');
		if(!$stmt->execute(array('dateOfVisit' => $_POST['dateOfVisit'], 'lname' => $_POST['lname'], 'fname' => $_POST['fname'], 'inHouse' => $_POST['inHouse'], 
						'phone' => $_POST['phone'], 'email' => $_POST['email'], 'program' => $_POST['program'], 'volunteer' => $_POST['volunteer'], 'numBags' => $_POST['numBags'], 'weight' => $_POST['weight'], 'id' => $_POST['id']))){
							 
			throw new PDOException('The execute method failed');
		}
		
		$stmt = $objDb->prepare('SELECT date_of_visit as dateOfVisit, program, volunteer, numBags, weight FROM visits WHERE client_id = :id');
